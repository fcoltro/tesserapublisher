...
fn migrate(value: &mut serde_json::Value, from: u32) {
    ...
    // refuses a document whose entries say "12–15" rather than indexing
    // the marker's page and saving that.
    ...
    // The version moves so an older build refuses a document whose lines
    // are justified by narrowing the type rather than setting them with
    // its spaces alone and saving that.
    ...
    // an older build refuses a document whose text says "see page 12"
    // through a marker it would draw as nothing.

    // 26 -> 27: a wrap may say which side the text runs on (`sides`). **No
    // step, on purpose**: absent, it reads as the largest area, which is
    // what every wrap did. The version moves so an older build refuses a
    // document whose text runs both sides of a picture rather than setting
    // it one side and saving that.

    // 25 -> 26: the document gained `footnotes`, its footnote options. **No
    // step, on purpose**: the defaults are what every note was set with
    // before there were options. The version moves so an older build
    // refuses a document whose notes count in roman or restart per page,
    // rather than numbering them 1, 2, 3 and saving that.
    ...
    // every page to the first on the next structural edit, which would
    // silently un-size a gatefold. Refusing is better than that.
    ...
    // that an older build refuses a document whose objects are anchored rather
    // than loading them as loose frames that no longer travel with their copy.
    ...
    // rewrite, and a step that touched every frame to write the value it would
    // already read as would be a migration that could only introduce a bug.
